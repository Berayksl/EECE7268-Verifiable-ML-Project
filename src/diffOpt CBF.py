...
if __name__ == "__main__":
    ...
    u_agent_max = 5 # Max vel. of the system

    point1 = (30, 0)
    point2 = (-30, 0)
    target_region_radius = 3
    agent_radius = 1
    u_target_max0 = 0.5

    obstacles = {
        0: {'label': 1,'center': (30, 0), 'radius': target_region_radius, 'u_max': u_target_max0, 'remaining_time': 100, 'movement':{'type': 'periodic', 'point1': point1, 'point2': point2, 'heading_angle': np.arctan2(point2[1] - point1[1], point2[0] - point1[0])}, 'color': 'blue'}
    ...
    config = {
        ...
        "dt": 1,
        "render": True,
        'dt_render': 0.02, #time interval for rendering
        "goals": goals,  # goal regions for the agent
        "obstacles": obstacles,  # dictionary of obstacles
        "targets": obstacles,  # dictionary of targets for the CBF
    ...
    while t <= episode_length:
        ...
        #u_cbf, slack = solve_cbf_qp(sequential_CBF, agent_state=state, u_agent_max=u_agent_max, disturbance_interval=[0,0], target_index=0, current_t=t, targets=obstacles, u_rl=action_rl)
